...
helpers do
  ...
  def card_image(card) # takes array-based card ['S','10']
   ...
   card = "<img src='/images/cards/#{suit}_#{value}.jpg' class='card_image'>"   
  end
  ...
  def winner!(msg)
    ...
    @winner = "<strong>#{session[:player_name]} wins $#{session[:player_bet]} and now has $#{session[:player_cash]}!</strong> #{msg}"
  end
  ...
  def loser!(msg)
    ...
    @play_again = true
    @show_hit_or_stay_buttons = false
    @loser = "<strong>#{session[:player_name]} loses $#{session[:player_bet]} and now has $#{session[:player_cash]}!</strong> #{msg}"
  end
...
post '/new_player' do
  if params[:player_name].empty?
    ...
    @error = "Non-alphanumeric characters detected. Enter a valid name."
    halt erb(:new_player)
  end
